...
def _build_graphsage_assets(
    ...
    for asin, g0 in work.groupby("asin", sort=False):
        ...
        # V16-TRENDGRAPH: long-horizon graph trend features.
        # These summarize whether each ASIN is rising/falling/stable before the forecast origin.
        # They are historical-only and are used as node + edge features, so the GAT can distinguish
        # stable positive neighbors from unstable/turnover-prone neighbors.
        def _log_slope_last(arr, w):
            arr = np.asarray(arr, dtype=float)
            if len(arr) < 2:
                return 0.0
            x = np.log1p(np.clip(arr[-min(int(w), len(arr)):], 0.0, None))
            if len(x) < 2 or np.nanstd(x) < 1e-8:
                return 0.0
            t = np.arange(len(x), dtype=float)
            t = (t - t.mean()) / (t.std() + 1e-8)
            return float(np.clip(np.polyfit(t, x, 1)[0], -5.0, 5.0))

        def _log_vol_last(arr, w):
            arr = np.asarray(arr, dtype=float)
            if len(arr) == 0:
                return 0.0
            x = np.log1p(np.clip(arr[-min(int(w), len(arr)):], 0.0, None))
            return float(np.clip(np.nanstd(x), 0.0, 5.0))

        def _active_rate_last(arr, w):
            arr = np.asarray(arr, dtype=float)
            if len(arr) == 0:
                return 0.0
            x = arr[-min(int(w), len(arr)):]
            return float(np.mean(x > 0)) if len(x) else 0.0
        ...
        # V16-TRENDGRAPH: short-vs-medium and medium-vs-long trend states.
        recent4_total_momentum = _safe_ratio(recent4_total_mean, recent13_total_mean)
        recent4_buybox_momentum = _safe_ratio(recent4_buybox_mean, recent13_buybox_mean)
        recent4_instock_momentum = _safe_ratio(recent4_instock_mean, recent13_instock_mean)
        recent4_demand_momentum = _safe_ratio(recent4_demand_mean, recent13_demand_mean)
        trend_short_momentum = float(np.mean([recent4_total_momentum, recent4_buybox_momentum, recent4_instock_momentum, recent4_demand_momentum]))
        trend_long_momentum = float(np.mean([recent13_total_momentum, recent13_buybox_momentum, recent13_instock_momentum, recent13_demand_momentum]))
        trend_momentum_gap = float(abs(trend_short_momentum - trend_long_momentum))

        total_slope13 = _log_slope_last(total, 13)
        buybox_slope13 = _log_slope_last(buy, 13)
        instock_slope13 = _log_slope_last(instock, 13)
        demand_slope13 = _log_slope_last(demand, 13)
        trend_slope13_mean = float(np.mean([total_slope13, buybox_slope13, instock_slope13, demand_slope13]))

        total_vol13 = _log_vol_last(total, 13)
        buybox_vol13 = _log_vol_last(buy, 13)
        instock_vol13 = _log_vol_last(instock, 13)
        demand_vol13 = _log_vol_last(demand, 13)
        trend_vol13_mean = float(np.mean([total_vol13, buybox_vol13, instock_vol13, demand_vol13]))

        active4_total = _active_rate_last(total, 4)
        active13_total = _active_rate_last(total, 13)
        active4_buybox = _active_rate_last(buy, 4)
        active13_buybox = _active_rate_last(buy, 13)
        active4_instock = _active_rate_last(instock, 4)
        active13_instock = _active_rate_last(instock, 13)
        active4_demand = _active_rate_last(demand, 4)
        active13_demand = _active_rate_last(demand, 13)
        trend_active_delta = float(np.mean([
            active4_total - active13_total, active4_buybox - active13_buybox,
            active4_instock - active13_instock, active4_demand - active13_demand
        ]))
        # Large means recent behavior is less stable; useful for h13-h20 decay.
        graph_relation_instability_proxy = float(np.clip(0.35 * abs(trend_momentum_gap) + 0.35 * trend_vol13_mean + 0.30 * abs(trend_active_delta), 0.0, 5.0))
        ...
        rows.append({
            ...
            # V16-TRENDGRAPH node features
            "recent4_total_momentum": recent4_total_momentum,
            "recent4_buybox_momentum": recent4_buybox_momentum,
            "recent4_instock_momentum": recent4_instock_momentum,
            "recent4_demand_momentum": recent4_demand_momentum,
            "trend_short_momentum": trend_short_momentum,
            "trend_long_momentum": trend_long_momentum,
            "trend_momentum_gap": trend_momentum_gap,
            "total_slope13": total_slope13,
            "buybox_slope13": buybox_slope13,
            "instock_slope13": instock_slope13,
            "demand_slope13": demand_slope13,
            "trend_slope13_mean": trend_slope13_mean,
            "total_vol13": total_vol13,
            "buybox_vol13": buybox_vol13,
            "instock_vol13": instock_vol13,
            "demand_vol13": demand_vol13,
            "trend_vol13_mean": trend_vol13_mean,
            "active4_total": active4_total,
            "active13_total": active13_total,
            "active4_buybox": active4_buybox,
            "active13_buybox": active13_buybox,
            "active4_instock": active4_instock,
            "active13_instock": active13_instock,
            "active4_demand": active4_demand,
            "active13_demand": active13_demand,
            "trend_active_delta": trend_active_delta,
            "graph_relation_instability_proxy": graph_relation_instability_proxy,
        })
    ...
    # V16-TRENDGRAPH: normalize trend states inside category so the graph sees
    # whether an ASIN is rising/falling relative to comparable category peers.
    for _c in ["trend_short_momentum", "trend_long_momentum", "trend_slope13_mean", "trend_vol13_mean", "graph_relation_instability_proxy", "trend_active_delta"]:
        feat[f"cat_rank_{_c}"] = 0.5
        if _c in feat.columns:
            for _, idxs in cat_series.groupby(cat_series).groups.items():
                idxs = list(idxs)
                feat.loc[idxs, f"cat_rank_{_c}"] = _pct_rank_group(feat.loc[idxs, _c]).values
    ...
    # V16-TRENDGRAPH: node-level trend context. These features are historical-only and help
    # the graph distinguish stable neighbors from rising/falling/unstable neighbors for h13-h20.
    trend_cols = [
        "recent4_total_momentum", "recent4_buybox_momentum", "recent4_instock_momentum", "recent4_demand_momentum",
        "trend_short_momentum", "trend_long_momentum", "trend_momentum_gap",
        "total_slope13", "buybox_slope13", "instock_slope13", "demand_slope13", "trend_slope13_mean",
        "total_vol13", "buybox_vol13", "instock_vol13", "demand_vol13", "trend_vol13_mean",
        "active4_total", "active13_total", "active4_buybox", "active13_buybox",
        "active4_instock", "active13_instock", "active4_demand", "active13_demand",
        "trend_active_delta", "graph_relation_instability_proxy",
        "cat_rank_trend_short_momentum", "cat_rank_trend_long_momentum", "cat_rank_trend_slope13_mean",
        "cat_rank_trend_vol13_mean", "cat_rank_graph_relation_instability_proxy", "cat_rank_trend_active_delta",
    ]
    node_feature_cols = list(dict.fromkeys(
        zero_cols + level_peak_cols + demand_cols + transition_cols + static_cols + brand_cols + trend_cols +
    ...
    # V16-TRENDGRAPH pairwise trend features.
    z_trend_short = _z_arr("trend_short_momentum")
    z_trend_long = _z_arr("trend_long_momentum")
    z_trend_slope = _z_arr("trend_slope13_mean")
    z_trend_vol = _z_arr("trend_vol13_mean")
    z_trend_instability = _z_arr("graph_relation_instability_proxy")
    z_trend_active_delta = _z_arr("trend_active_delta")
    ...
    def _relation_components(i, cand):
        ...
        # V16-TRENDGRAPH: compare short-term trend, long-term trend, slope, and instability.
        trend_short_gap = np.abs(z_trend_short[cand] - z_trend_short[i])
        trend_long_gap = np.abs(z_trend_long[cand] - z_trend_long[i])
        trend_slope_gap = np.abs(z_trend_slope[cand] - z_trend_slope[i])
        trend_active_gap = np.abs(z_trend_active_delta[cand] - z_trend_active_delta[i])
        trend_instability_gap = np.abs(z_trend_instability[cand] - z_trend_instability[i])
        trend_instability_mean = 0.5 * (np.abs(z_trend_instability[cand]) + np.abs(z_trend_instability[i]))
        # same direction if both have similar sign in recent slope/momentum.
        trend_same_direction = (np.sign(z_trend_slope[cand] + 1e-8) == np.sign(z_trend_slope[i] + 1e-8)).astype(float)
        trend_opposite_direction = 1.0 - trend_same_direction
        trend_gap = (trend_short_gap + trend_long_gap + trend_slope_gap + trend_active_gap) / 4.0
        ...
        pos_score = (
            ...
            - 0.45 * price_gap
            - 0.55 * trend_gap
            - 0.25 * trend_instability_mean
            + 0.25 * trend_same_direction
            - 0.75 * pkg_distance
            - 0.45 * pkg_volume_gap
            - 0.35 * pkg_weight_gap
        ...
        comp_score = (
            ...
            + 0.35 * price_gap
            + 0.45 * trend_gap
            + 0.25 * trend_opposite_direction
            + 0.20 * trend_instability_gap
            + 0.45 * promo_gap
            + 0.20 * prime_gap
        ...
        return pos_score, comp_score, {
            ...
            "zero_gap": zero_gap,
            "price_gap": price_gap,
            "trend_short_gap": trend_short_gap,
            "trend_long_gap": trend_long_gap,
            "trend_slope_gap": trend_slope_gap,
            "trend_active_gap": trend_active_gap,
            "trend_instability_gap": trend_instability_gap,
            "trend_instability_mean": trend_instability_mean,
            "trend_same_direction": trend_same_direction,
            "trend_opposite_direction": trend_opposite_direction,
            "trend_gap": trend_gap,
    ...
    edge_feature_names = [
        ...
        "trend_short_gap", "trend_long_gap", "trend_slope_gap", "trend_active_gap",
        "trend_instability_gap", "trend_instability_mean", "trend_same_direction",
        "trend_opposite_direction", "trend_gap",
    ...
    rank_feature_cols = [
        ...
        # V16-TRENDGRAPH rank side-channel features
        "trend_short_momentum", "trend_long_momentum", "trend_slope13_mean", "trend_vol13_mean",
        "trend_active_delta", "graph_relation_instability_proxy",
        "cat_rank_trend_short_momentum", "cat_rank_trend_long_momentum", "cat_rank_trend_slope13_mean",
        "cat_rank_trend_vol13_mean", "cat_rank_graph_relation_instability_proxy", "cat_rank_trend_active_delta",
    ]
...
class ExposureForecastModelV2(nn.Module):
    ...
    def __init__(self, input_dim, context_dim,
        ...
        if self.use_graphsage:
            ...
            print(f"V16 trend graph: sparse/event-aware dynamic rank + package-aware edge + trend-gap edge features for long horizon.")
        else:
...
def run_exposure_v2(
    ...
    print("Preset: V16 PKG-TREND graph + sparse/event-aware rank prior + long-horizon calibration; MU hats for demand")
    print("=" * 100)
