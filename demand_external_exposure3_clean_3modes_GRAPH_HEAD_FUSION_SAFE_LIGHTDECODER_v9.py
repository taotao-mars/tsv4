...
def load_real_data(data_raw, dph_cap_q=0.995):
    ...
    # Business assumption: future promotion index / rate is available at forecast origin.
    # These are allowed future-known context features, not leakage under that assumption.
    # ------------------------------------------------------------
    if "ind_promotion" in data_raw.columns:
        data_raw["known_promo_index"] = pd.to_numeric(data_raw["ind_promotion"], errors="coerce").fillna(0.0).clip(0, 1)
    else:
        data_raw["known_promo_index"] = 0.0

    if "promotion_ratio" in data_raw.columns:
        data_raw["known_promo_rate"] = pd.to_numeric(data_raw["promotion_ratio"], errors="coerce").fillna(0.0).clip(lower=0.0)
    else:
        data_raw["known_promo_rate"] = 0.0

    if "promotion_amount" in data_raw.columns:
        data_raw["known_promo_amount_log"] = np.log1p(pd.to_numeric(data_raw["promotion_amount"], errors="coerce").fillna(0.0).clip(lower=0.0))
    else:
        data_raw["known_promo_amount_log"] = 0.0

    if "promotion_pricing_amount" in data_raw.columns:
        data_raw["known_promo_price_amount_log"] = np.log1p(pd.to_numeric(data_raw["promotion_pricing_amount"], errors="coerce").fillna(0.0).clip(lower=0.0))
    else:
        data_raw["known_promo_price_amount_log"] = 0.0

    if "promotion_type" in data_raw.columns:
        _ptype = data_raw["promotion_type"].astype(str).fillna("NO_PROMO")
        _codes, _uniques = pd.factorize(_ptype)
        data_raw["known_promo_type_code"] = (_codes.astype(float) / max(len(_uniques) - 1, 1))
        data_raw.loc[data_raw["known_promo_index"] <= 0.5, "known_promo_type_code"] = 0.0
    else:
        data_raw["known_promo_type_code"] = 0.0

    if "pricing_type" in data_raw.columns:
        _prtype = data_raw["pricing_type"].astype(str).fillna("NO_PRICE_TYPE")
        _pcodes, _puniques = pd.factorize(_prtype)
        data_raw["known_pricing_type_code"] = (_pcodes.astype(float) / max(len(_puniques) - 1, 1))
    else:
        data_raw["known_pricing_type_code"] = 0.0

    known_promo_context_cols = [
        "known_promo_index",
        "known_promo_rate",
    ...
    context_cols = ["our_price"] + holiday_cols + distance_cols + seasonal_cols + proximity_cols + known_promo_context_cols
...
class TCNSparseAttnEncoder(nn.Module):
    ...
    def encode_sequence(self, x):
        """Return full historical encoder states plus the final latent.

        Kept separate from forward() so existing diagnostics that call
        `mu, alpha, h_t = model.encoder(x)` remain backward compatible.
        """
        b_t        = x[:, :, 1]
        ...
        h_seq = self.sparse_attn(h.permute(0, 2, 1), b_t, peak_score)
        h_t   = self.final_norm(h_seq[:, -1, :])
        return h_seq, h_t, b_t, peak_score

    def forward(self, x):
        h_seq, h_t, b_t, peak_score = self.encode_sequence(x)
...
class DemandLightDecoderResidual(nn.Module):
    """
    Zero-start one-layer cross-attention residual for demand.

    Purpose:
      - Keep the original v8 demand head as the main predictor.
      - Let each future horizon query historical demand encoder states using
        known future context and predicted exposure hats.
      - Only correct log_mu with a small residual. Alpha is intentionally
        untouched for stability.
    """
    def __init__(self, d_model=32, context_dim=2, horizon=20,
                 n_heads=4, residual_scale=0.03,
                 active_bias=0.75, peak_bias=0.50):
        super().__init__()
        assert d_model % n_heads == 0
        self.d_model = d_model
        ...
        # zero-start: v9 begins as exactly v8 for the decoder residual path.
        final_layer = self.delta_head[-1]
        nn.init.zeros_(final_layer.weight)
        nn.init.zeros_(final_layer.bias)

    def forward(self, enc_seq, future_context, hist_active=None, hist_peak=None):
        B, T, D = enc_seq.shape
        H = future_context.shape[1]
        q0 = self.future_query(future_context)  # [B,H,D]
        q = self.q_proj(q0).view(B, H, self.n_heads, self.d_head).transpose(1, 2)
        k = self.k_proj(enc_seq).view(B, T, self.n_heads, self.d_head).transpose(1, 2)
        v = self.v_proj(enc_seq).view(B, T, self.n_heads, self.d_head).transpose(1, 2)

        scores = torch.matmul(q, k.transpose(-2, -1)) / np.sqrt(self.d_head)  # [B,heads,H,T]
        if hist_active is not None:
            scores = scores + self.active_bias * hist_active.float()[:, None, None, :]
        if hist_peak is not None:
            peak_norm = hist_peak / (hist_peak.max(dim=1, keepdim=True)[0] + 1e-6)
            scores = scores + self.peak_bias * peak_norm[:, None, None, :]

        attn = torch.softmax(scores, dim=-1)
        out = torch.matmul(attn, v)
        out = out.transpose(1, 2).contiguous().view(B, H, D)
        out = self.out_norm(out + q0)
        delta = self.delta_head(out).squeeze(-1) * self.residual_scale  # [B,H]
        return delta
...
class TCN_ENN(nn.Module):
    ...
    v9 adds a zero-start light demand decoder residual.
    It is NOT a full decoder; it only applies a small horizon-specific
    cross-attention correction to log_mu.
    ...
    def __init__(self, input_dim=34, context_dim=2, d_model=32,
        ...
        # Graph-head fusion: package-aware graph columns are not appended to raw encoder x.
        # They are projected from future_context, fused with encoder latent h_t, and then used by
        # z_generator, epinet, and final base-head corrections. This implements:
        #   Head(h_t, graph, z, known_future_context).
        ctx_cols = globals().get("_LAST_DEMAND_CONTEXT_COLS", [])
        self.graph_feat_indices = [
        ...
        if len(self.graph_feat_indices) > 0:
            ...
            self.graph_base_delta_head = nn.Sequential(
                nn.Linear(d_model, 64), nn.ReLU(), nn.Dropout(0.10),
        ...
        # v9: conservative demand-side decoder residual. It only corrects log_mu.
        self.light_decoder_scale = 0.03
        self.light_decoder = DemandLightDecoderResidual(
            d_model=d_model,
            context_dim=context_dim,
            horizon=horizon,
            n_heads=4,
            residual_scale=self.light_decoder_scale,
        )
        print(f"Demand v9 light decoder residual enabled | scale={self.light_decoder_scale} | zero_start=True | alpha_unchanged=True")
    ...
    def forward(self, x, future_context, nZ=8, *args, **kwargs):
        h_seq, h_t, hist_active, hist_peak = self.encoder.encode_sequence(x)
        mu_base = F.softplus(self.encoder.base_head(h_t))
        alpha_base = F.softplus(self.encoder.alpha_head(h_t)) + 1e-4
        h_fused, graph_emb, graph_mu_delta, graph_alpha_delta = self._graph_fuse(h_t, future_context)
        if graph_mu_delta is not None:
        ...
        # v9 light decoder: horizon-specific residual on log_mu only.
        decoder_mu_delta = self.light_decoder(h_seq, future_context, hist_active, hist_peak)
        mu_base = F.softplus(torch.log(mu_base + 1e-6) + decoder_mu_delta)
...
def load_real_data(data_raw, dph_cap_q=0.995):
    ...
    print("New context dim:", context_dim)
    print("=" * 100)
    globals()["_LAST_DEMAND_CONTEXT_COLS"] = list(context_cols)
    globals()["_LAST_DEMAND_GRAPH_CONTEXT_COLS"] = [c for c in GRAPH_CONTEXT_COLS if c in context_cols]
    return data, context_dim, context_cols
