...
class DemandDataset(Dataset):
    ...
    def _make_future_context_with_dph_proxies(self, d, start, history, horizon):
        ...
        # ------------------------------------------------------------------
        # V11 strict fix: origin-safe category graph context.
        #
        # The graph columns are precomputed per ASIN-week from lagged rolling
        # demand/DPH profiles. If we simply slice
        #   future_context[start+history : start+history+horizon]
        # then h2...h20 may contain graph features computed using earlier
        # weeks inside the current forecast horizon. That leaks target-horizon
        # demand/DPH.
        #
        # Correct behavior for a rolling forecast origin is:
        #   1) use the graph profile at the origin row only
        #      (row start+history, whose lagged rolling stats use history only),
        #   2) repeat that same profile across all horizon steps.
        #
        # This keeps category graph edges/static features, but prevents graph
        # node/profile features from updating with unknown future values.
        # ------------------------------------------------------------------
        try:
            gidx = globals().get("DEMAND_GRAPH_CONTEXT_IDX", [])
            if len(gidx) > 0 and fc.shape[1] > max(gidx):
                origin_row = min(start + history, d["future_context"].shape[0] - 1)
                origin_graph = d["future_context"][origin_row, gidx].astype(np.float32)
                fc[:, gidx] = origin_graph[None, :]
        except Exception as e:
            # Do not break data loading; if this ever fires, print once per sample path
            # would be too noisy, so keep silent and rely on downstream shape checks.
            pass
...
def _make_origin_safe_category_graph_feature_frame(data_raw, hist_weeks=13):
    ...
    # V11 strict-safe scaling: do NOT use full-sample mean/std here.
    # Using all weeks to compute normalization statistics is a mild preprocessing
    # leakage in rolling backtests. These features are already log/ratio scaled;
    # we only clip to stable ranges with fixed constants.
    for c in graph_cols:
        val = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
        if c == "graph_own_active13":
            val = val.clip(0.0, 1.0)
        elif c in {"graph_rel_demand_strength", "graph_rel_exposure_strength"}:
            val = val.clip(-5.0, 5.0)
        else:
            val = val.clip(0.0, 12.0)
...
def load_real_data(data_raw, dph_cap_q=0.995):
    ...
    if added_any:
        ...
        print("\n" + "=" * 100)
        print("V11 CATEGORY-LEVEL GRAPH FEATURES APPENDED; DATASET REPEATS ORIGIN PROFILE")
        print("=" * 100)
        print("Graph neighborhood: category_code only; GL remains static/context feature if present. Graph profile is repeated from forecast origin inside Dataset.")
...
# ==================================================================================================
# V11 EXTENSION: ORIGIN-SAFE HISTORICAL-DPH-ONLY DIRECT DEMAND ABLATION
# --------------------------------------------------------------------------------------------------
...
def run_demand_histdph_only_v11(
    data_raw1,
    scot_df,
    demand_head_type="patch",
    use_category_graph_features=True,
    ...
    """
    V11 direct demand run: historical DPH only, no predicted future DPH, origin-safe graph context.
    ...
    print("\n" + "=" * 100)
    print("V11 DIRECT DEMAND | HISTORICAL DPH ONLY | ORIGIN-SAFE GRAPH | NO PREDICTED FUTURE DPH")
    print("=" * 100)
    ...
    print("Category graph profile: origin-safe repeat across h1-h20; no horizon-internal update.")
    print("remove_extreme default: False, to avoid full-sample demand peak filtering leakage.")
...
def run_demand_direct_vs_patch_ablation_histdph_only_v11(
    data_raw1,
    scot_df,
    n_asins=5000,
    seed=42,
    ...
    """
    print("\n" + "#" * 100)
    print("RUNNING V11 ORIGIN-SAFE HIST-DPH-ONLY ABLATION A: NO-DECODER / DIRECT HORIZON HEAD")
    print("#" * 100)
    result_direct = run_demand_histdph_only_v11(
        data_raw1=data_raw1,
        scot_df=scot_df,
        demand_head_type="direct",
        use_category_graph_features=True,
    ...
    print("\n" + "#" * 100)
    print("RUNNING V11 ORIGIN-SAFE HIST-DPH-ONLY ABLATION B: PATCH DECODER HEAD")
    print("#" * 100)
    result_patch = run_demand_histdph_only_v11(
        data_raw1=data_raw1,
        scot_df=scot_df,
        demand_head_type="patch",
        use_category_graph_features=True,
    ...
    out = {
        "direct_histdph_only_origin_safe": result_direct,
        "patch_histdph_only_origin_safe": result_patch,
    }

    print("\n" + "=" * 100)
    print("V11 ORIGIN-SAFE HIST-DPH-ONLY ABLATION FINISHED")
    print("=" * 100)
    print("Result keys: out['direct_histdph_only_origin_safe'], out['patch_histdph_only_origin_safe']")
    print("Both results keep original result['forecast_df'] and result['real_scot_outputs'] format.")
    print("Use the existing SCOT WAPE outputs exactly as before.")
    return out
...
"""
V11 USAGE IN JUPYTER
--------------------
%run -i demand_direct_categorygraph_patchdecoder_histdphonly_ORIGINSAFE_v11.py

# Single patch-decoder run: no predicted future DPH, historical DPH only.
demand_result_patch_histdph_v11 = run_demand_histdph_only_v11(
    data_raw1=data_raw1,
    scot_df=scot_df,
    demand_head_type="patch",
    graph_head_scale=0.03,
...
demand_ablation_histdph_v11 = run_demand_direct_vs_patch_ablation_histdph_only_v11(
    data_raw1=data_raw1,
    scot_df=scot_df,
    n_asins=5000,
    epochs=30,
