...
# ============================================================
# V25 save exposure hats for demand reuse
# ============================================================

def save_exposure_hat_for_demand_csv(exposure_result_or_hat, csv_path="/mnt/data/exposure_v25_hat_for_demand.csv"):
    """
    Save exposure_hat_for_demand to CSV so demand can be run later without rerunning exposure.

    Accepts either:
      - full exposure result dict containing ['exposure_hat_for_demand']
      - direct exposure hat dataframe

    Output columns are kept compatible with demand readers:
      asin, order_week, pred_total_dph, pred_buy_box_dph, pred_instock_dph
    Additional columns, if present, are preserved.
    """
    import os
    import pandas as pd
    import numpy as np
    ...
    # Normalize date / id types for robust later merge.
    hat["asin"] = hat["asin"].astype(str)
    hat["order_week"] = pd.to_datetime(hat["order_week"])

    # Ensure standard level prediction columns exist if log columns are the only available ones.
    ...
    preferred = ["asin", "order_week", "pred_total_dph", "pred_buy_box_dph", "pred_instock_dph"]
    ...
    print("Columns:", hat.columns.tolist())
    return csv_path
